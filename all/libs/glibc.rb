class Glibc < Formula
  ...
  def install
    mkdir "build" do
      ...
      gcc_version = `ls /usr/bin/ | grep gcc`.chomp.split("\n")
      if gcc_version.include? "gcc-4.6" and gcc_version.include? "gcc-4.7"
          args << "CC=/usr/bin/gcc-4.6"
          args << "CXX=/usr/bin/g++-4.6"
      end
  ...
  def post_install
    ...
    locales.uniq.each do |locale|
      ...
      if !charmap.nil?
        print bin/"localedef", "-i", lang, "-f", charmap, locale
        system bin/"localedef", "-i", lang, "-f", charmap, locale
      else
        print bin/"localedef", "-i", lang, locale
